<template>
	<view class="page-container h-screen overflow-y-auto">
		...
		<view class="scroll-container">
			...
			<view class="bg-yellow-950 h-screen scroll-container flex justify-center items-center">
				<view class="bg-green-100 h-3/5 w-4/5 flex gap-x-0">
					...
					<view class="bg-yellow-200 h-full w-2/5 ml-[calc(2/15*100%)]">
						...
						<u-button type="primary" @click="next(0)">next</u-button>
						<CustomToast v-if="showToast" :message="toastMessage" :width="toastWidth"
							:height="toastHeight" />
			...
			<!-- 数字选择弹窗 -->
			<view v-if="showNumberModal" class="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50">
				<view class="bg-white p-4 rounded-lg">
					<!-- 显示累加和 -->
					<view class="text-center mb-2">{{ sumDisplay }}</view>
					<!-- 数字选择区域 -->
					<view class="grid grid-cols-5 gap-2">
						<button v-for="num in 10" :key="num" @click="addNumber(num - 1)"
					...
					<view class="flex justify-between mt-4">
						<button @click="showNumberModal = false; clearSelectedNumbers()"
							class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
							取消
						</button>
						<button @click="saveData(); showNumberModal = false; clearSelectedNumbers()"
...
<script>
	...
	export default {
		...
		computed: {
			aWinRound() {
				return this.authStore.doubleMode.a_win_round;
			},
			bWinRound() {
				return this.authStore.doubleMode.b_win_round;
			},
			doneRound() {
				return this.authStore.doubleMode.done_round;
			},
			updateTimeWords() {
		...
		methods: {
			find() {
				console.log('55556879', this.authStore.doubleMode)
			},
			back() {
				if (!this.authStore.doubleMode.records.length) {
					if (!this.authStore.doubleMode.a_score && !this.authStore.doubleMode.b_score) {
						this.showCustomToast("empty records")
					} else {
						this.showCustomToast("max times is three")
					}
				}

				let latest_ele = this.authStore.doubleMode.records.pop()
        console.log(9999,latest_ele)
        if(latest_ele === null || typeof latest_ele === 'undefined'){
          return
        }
				if (!Object.keys(latest_ele).length) {
					return
				}
				this.authStore.setDoubleMode({
					'a_score': latest_ele.a_score,
					'a_max': latest_ele.a_max,
					'user_index': latest_ele.user_index,
					'b_score': latest_ele.b_score,
					'b_max': latest_ele.b_score,
				})
			},
			next(value = 0) { // 0 不结束  1结束
				...
				// 历史数组追加
				let fight_history = this.authStore.doubleMode.fight_history


				console.log(22222, this.authStore.doubleMode, fight_history)
				// return
				...
				this.authStore.setDoubleMode({
					...
					double_timer_status: value ? 0 : 1,
					is_finish: value,
					records: []
				})
				console.log("hasjd8888888", this.authStore.doubleMode)
			...
			// 清零
			clearData() {
				this.authStore.initDoubleMode()
				console.log("设置后的值：", this.authStore.doubleMode)
			},
        ...
        // 记录最多三条
        let record = this.authStore.doubleMode.records
        record.push({
          'a_score': this.authStore.doubleMode.a_score,
          'a_max': this.authStore.doubleMode.a_max,
          'user_index': this.authStore.doubleMode.user_index,
          'b_score': this.authStore.doubleMode.b_score,
          'b_max': this.authStore.doubleMode.b_max,
        })
        if (record.length > 3) {
          record.shift()
			...
			// 保存数据
			saveData() {
				console.log(22222111111, this.authStore.doubleMode["total_round"]);
				this.authStore.setDoubleMode({
